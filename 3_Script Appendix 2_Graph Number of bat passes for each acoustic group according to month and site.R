...
Hippo_ac <- selected_rows  %>%
  filter(SpMaxF2 %in% "CF-FMd") |>
  dplyr::filter(dplyr::between(MainMode, 125, 160)) |> # select H sp
...
Minio_QCF <- selected_rows  %>%
  filter(SpMaxF2 %in% "QCF-FMd") |>
  dplyr::filter(dplyr::between(MainMode, 55, 65)) |> 
  dplyr::filter(dplyr::between(Ramp90, -20, -0.1)) |> 
  dplyr::filter(dplyr::between(Ind, 0.2, 1)) 

Minio_all <- merge(Minio_35107kHz, Minio_QCF, all.x = T, all.y=T)

Minio_Allac <- Minio_all[!duplicated(Minio_all[c('Sites', 'Date_Night2')]), ]
...
Rum_lan_alcy <- selected_rows  %>%
  filter(SpMaxF2 %in% "FMu-CF-FMd") |>
  filter(Sites %in% "Mont Belo") |>
  dplyr::filter(dplyr::between(MainMode, 45, 62)) |> 
  dplyr::filter(dplyr::between(Ramp90, -20, 0))

Rum_lan_alcy_om <- selected_rows  %>%
  filter(SpMaxF2 %in% "other-mammal") |>
  filter(Sites %in% "Mont Belo") |>
  dplyr::filter(dplyr::between(MainMode, 45, 62)) |> 
  dplyr::filter(dplyr::between(Ramp90, -20, 0))

Rum_lan_alcy_all <- merge(Rum_lan_alcy, Rum_lan_alcy_om, all.x = T, all.y=T)

Rum_lan_alcy_Allac <- Rum_lan_alcy_all[!duplicated(Rum_lan_alcy_all[c('Sites', 'Date_Night2')]), ]
...
Rhino_denti <- selected_rows  %>%
  filter(SpMaxF2 %in% "FMu-CF-FMd") |>
  dplyr::filter(dplyr::between(MainMode, 95, 110)) |> 
  dplyr::filter(dplyr::between(Ramp90, -20, -0.1)) |> 
  dplyr::filter(dplyr::between(Ind, 0.17, 1)) 

Rhino_denti_om <- selected_rows  %>%
  filter(SpMaxF2 %in% "other-mammal") |>
  dplyr::filter(dplyr::between(MainMode, 95, 110)) |> 
  dplyr::filter(dplyr::between(Ramp90, -20, 0))

Rhino_denti_all <- merge(Rhino_denti, Rhino_denti_om, all.x = T, all.y=T)

Rhino_denti_Allac <- Rhino_denti_all[!duplicated(Rhino_denti_all[c('Sites', 'Date_Night2')]), ]
